...
async function buttonHandler(interaction) {
  ...
  try {
    ...
    if (customId.startsWith('edital_confirmar:')) {
      return await editalController.handleConfirmarCanal(interaction);
    }
    if (customId.startsWith('edital_voltar:')) {
      return await editalController.handleVoltarEdital(interaction);
    }
    if (customId.startsWith('edital_editar:')) {
      return await editalController.handleEditarEdital(interaction);
    }
    if (customId.startsWith('edital_desistir:')) {
      return await editalController.handleDesistirEdital(interaction);
    }
    if (customId.startsWith('edital_concordo:')) {
      return await editalController.handleConcordarEdital(interaction);
    }
    if (customId.startsWith('edital_discordo:')) {
      return await editalController.handleDiscordarEdital(interaction);
    }
    ...
    // ==========================================
    // ROTEAMENTO DA FORÇA TÁTICA
    // ==========================================
    if (base === 'ft_ver_efetivo') {
      await interaction.deferReply(EPHEMERAL_REPLY);
      const FT_ROLE_ID = '1510829677357432853';
      const members = await interaction.guild.members.fetch();
      const ftMembers = members.filter(m => m.roles.cache.has(FT_ROLE_ID));

      const lines = [];
      for (const [, member] of ftMembers) {
        const nick = member.nickname || member.user.displayName || member.user.username;
        lines.push(`> <@${member.id}> — \`${nick}\``);
      }

      const componentFactory = require('../utils/componentFactory');
      return interaction.editReply(componentFactory.createFtEfetivoPayload(lines, ftMembers.size));
    }

    if (base === 'ft_ver_hierarquia') {
      await interaction.deferReply(EPHEMERAL_REPLY);
      try {
        const hierarchyService = require('../services/hierarchyService');
        const corporationsConfig = require('../config/corporations');
        const ftTag = corporationsConfig.tags.find(t => t.slug === 'ft');

        if (!ftTag) {
          return interaction.editReply({ content: '❌ Tag da Força Tática não encontrada na configuração.' });
        }
        ...
        const ranks = corporationsConfig.getResolvedRanks('ft');
        const parentCorp = corporationsConfig.getPrimaryCorporation('ft');

        // Agrupar membros por patente
        const grouped = new Map();
        for (const [, member] of ftMembers) {
          let highestRank = null;
          // Verificar ranks da FT no banco de dados (Corporation model)
          const Corporation = require('../database/models/Corporation');
          const ftCorp = await Corporation.findOne({ slug: 'ft', guildId: interaction.guildId });

          if (ftCorp && ftCorp.ranks) {
            const matching = ftCorp.ranks.filter(r => r.roleId && member.roles.cache.has(r.roleId));
          ...
          // Fallback: tentar ranks da PMESP (corp-mãe)
          if (!highestRank && parentCorp) {
            const pmespCorp = await Corporation.findOne({ slug: 'pmesp', guildId: interaction.guildId });
            if (pmespCorp && pmespCorp.ranks) {
              const matching = pmespCorp.ranks.filter(r => r.roleId && member.roles.cache.has(r.roleId));
              if (matching.length > 0) {
                matching.sort((a, b) => b.level - a.level);
                highestRank = matching[0];
              }
          ...
          if (!highestRank) highestRank = { name: 'Oficial', level: -1, emoji: '👮' };

          const key = `${highestRank.level}::${highestRank.name}`;
          if (!grouped.has(key)) grouped.set(key, { rank: highestRank, members: [] });
          grouped.get(key).members.push(member);
        }

        // Ordenar por level decrescente
        const sorted = [...grouped.values()].sort((a, b) => b.rank.level - a.rank.level);

        const lines = [];
        for (const group of sorted) {
          const rankEmoji = group.rank.emoji || '👮';
          lines.push(`\n${rankEmoji} **${group.rank.name}** — ${group.members.length} membro(s)`);
        ...
        const container = new ContainerBuilder()
          ...
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(
              `⚡ **Hierarquia da Força Tática**\n\n` +
              `> **Total de membros:** \`${ftMembers.size}\`\n` +
        ...
        logger.error('Erro ao exibir hierarquia FT:', err);
        return interaction.editReply({ content: '❌ Ocorreu um erro ao buscar a hierarquia da Força Tática.' });
      }
    }
