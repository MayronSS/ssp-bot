 ...
 * Fallback: Administrador ou cargos com nomes comuns de Comando/Direção/Staff.
 */
async function canSetupPanels(member) {
  ...
  const corpComandoRoles = await getAllCorpRoles(guildId, 'comando');
  if (hasAnyRole(member, corpComandoRoles)) return true;

  return member.roles.cache.some(r =>
    ['comando', 'direcao', 'alto comando', 'staff', 'setup', 'administracao', 'administrativo'].some(name =>
      r.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(name)
    )
  );
}
