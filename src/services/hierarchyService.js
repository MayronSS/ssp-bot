...
async function performUpdate(guild) {
  try {
    ...
    // 3. Buscar TODOS os membros do Discord (fetch completo obrigatório)
    let members;
    try {
      members = await guild.members.fetch();
    } catch (fetchErr) {
      logger.warn(`Erro ao fazer fetch completo dos membros, usando cache como fallback: ${fetchErr.message}`);
    ...
    for (const m of members.values()) {
      ...
      // B. Verificar corporações primárias (sem break — permite membro dual PMESP+PCESP)
    ...
    logger.info(`Hierarquia: ${members.size} membros processados, ${unclassifiedCount} não classificados (bots/cidadãos sem corporação).`);
