...
module.exports = {
  ...
  createFtPanelPayload,
  createFtEfetivoPayload,
};

// ==========================================
// MÓDULO DE FORÇA TÁTICA (FT)
// ==========================================
function createFtPanelButtons() {
  const emojiHelper = require('./emojiHelper');
  return createButtonRow([
    {
      customId: 'ft_ver_efetivo',
      label: 'Ver Efetivo',
      emoji: emojiHelper.getRaw('user') || '👥',
      style: ButtonStyle.Secondary,
    },
    {
      customId: 'ft_ver_hierarquia',
      label: 'Hierarquia FT',
...
function createFtPanelContainer() {
  const accentColor = 0x2E7D32; // Verde FT
  ...
  addLogoThumbnailToContainer(
    container,
    `${withWhiteIcon('star', '**⚡ FORÇA TÁTICA — PMESP**')}\n\n` +
    '> A **Força Tática** é a unidade de elite da Polícia Militar do Estado de São Paulo, ' +
    'especializada em operações de alta complexidade, patrulhamento tático e combate ao crime organizado.\n\n' +
    '• *Utilize os botões abaixo para consultar o efetivo ativo e a hierarquia do batalhão.*\n' +
    '• *Apenas membros da FT possuem acesso às funcionalidades deste painel.*'
  );
  ...
  container
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${withWhiteIcon('idcard', '**Informações do Batalhão**')}\n` +
        '> **Sigla:** `FT`\n' +
        '> **Corporação-Mãe:** `PMESP`\n' +
        '> **Especialidade:** Operações Táticas e Patrulhamento Ostensivo\n' +
        `> **Cargo:** <@&1510829677357432853>`
      )
    )
    .addSeparatorComponents(
      new SeparatorBuilder()
    ...
    .addActionRowComponents(createFtPanelButtons());

  return container;
}

function createFtPanelPayload() {
  return {
    components: [createFtPanelContainer()],
    flags: MessageFlags.IsComponentsV2,
  };
}

function createFtEfetivoPayload(memberLines = [], totalCount = 0) {
  const container = new ContainerBuilder()
    .setAccentColor(0x2E7D32);

  const body = memberLines.length > 0
    ? memberLines.join('\n')
    : '*Nenhum membro da Força Tática encontrado no momento.*';

  container
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${withWhiteIcon('user', '**⚡ Efetivo da Força Tática**')}\n\n` +
