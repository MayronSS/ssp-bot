class PfPanelApp {
  ...
  async renderPontoPage(container, silent = false) {
    ...
    try {
      ...
      const period = this.state.pontoPeriod || 'all';
      const advRoleId = this.state.pontoAdvancedRoleId || '';
      
      // Build advanced stats query
      let advQuery = `?period=${period}`;
      if (corpFilter) advQuery += `&corporationSlug=${corpFilter}`;
      if (advRoleId) advQuery += `&roleId=${advRoleId}`;
      if (period === 'custom') {
        if (this.state.pontoFilters.startDate) advQuery += `&startDate=${this.state.pontoFilters.startDate}`;
        if (this.state.pontoFilters.endDate) advQuery += `&endDate=${this.state.pontoFilters.endDate}`;
      }

      const [advancedRes, pontosRes, officersRes] = await Promise.all([
        this.api.fetch(`/ponto/stats/advanced${advQuery}`),
      ...
      if (!advancedRes.success || !pontosRes.success) {
        container.innerHTML = this.templates.errorPage('Erro ao carregar dados do Bate-Ponto.');
        return;
      }

      container.innerHTML = this.templates.pontoPage(advancedRes, pontosRes.pontos, this.state.pontoFilters, officersRes.officers || [], advancedRes.roles || []);
      this.setupPontoListeners(advancedRes, pontosRes.pontos, officersRes.officers || [], advancedRes.roles || []);
      
      // Render daily activity chart
      this.renderPontoChart(advancedRes.dailyActivity);
    } catch (err) {
      console.error(err);
  ...
  renderPontoChart(dailyActivity) {
    const canvas = this.root.querySelector('#ponto-daily-chart');
    if (!canvas || !dailyActivity) return;

    // Destroy previous chart
    if (this.state.charts.pontoDaily) {
      this.state.charts.pontoDaily.destroy();
    }
    ...
    const hoursData = Object.values(dailyActivity).map(d => d.hours);
    const shiftsData = Object.values(dailyActivity).map(d => d.shifts);

    const isDark = this.state.theme === 'dark';
    const gridColor = isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.08)';
    const textColor = isDark ? '#8a93a6' : '#5c6478';

    this.state.charts.pontoDaily = new Chart(canvas.getContext('2d'), {
      type: 'bar',
      data: {
        labels,
        datasets: [
          {
            label: 'Horas',
            data: hoursData,
            backgroundColor: 'rgba(27, 82, 241, 0.7)',
            borderColor: 'rgba(27, 82, 241, 1)',
            borderWidth: 1,
            borderRadius: 6,
            barPercentage: 0.7,
            yAxisID: 'y'
      ...
      options: {
        ...
        plugins: {
          legend: { display: true, position: 'top', labels: { color: textColor, font: { size: 10, weight: '600' }, usePointStyle: true, pointStyleWidth: 8 } },
          tooltip: {
            backgroundColor: isDark ? '#1a2236' : '#ffffff',
            titleColor: isDark ? '#e8ecf2' : '#0a0f1c',
            bodyColor: isDark ? '#8a93a6' : '#5c6478',
            borderColor: isDark ? '#2a3450' : '#dfe3ea',
            borderWidth: 1,
            cornerRadius: 12,
            padding: 12,
        ...
        scales: {
          x: { grid: { display: false }, ticks: { color: textColor, font: { size: 9, weight: '600' } } },
          y: { position: 'left', grid: { color: gridColor }, ticks: { color: textColor, font: { size: 9 }, callback: v => v + 'h' }, beginAtZero: true },
          y1: { position: 'right', grid: { display: false }, ticks: { color: 'rgba(16, 185, 129, 0.8)', font: { size: 9 } }, beginAtZero: true }
        }
      }
    });
  }
  ...
  setupPontoListeners(stats, pontos, officers, roles) {
    ...
    this.root.querySelectorAll('.ponto-period-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        this.state.pontoPeriod = btn.dataset.period;
        this.renderPontoPage(this.root.querySelector('#page-content'));
      });
    });

    // Advanced role filter (patente)
    const advRoleSelect = this.root.querySelector('#ponto-adv-role');
    advRoleSelect?.addEventListener('change', () => {
      this.state.pontoAdvancedRoleId = advRoleSelect.value;
      this.renderPontoPage(this.root.querySelector('#page-content'));
    });

    // Corporation filter in stats area
    const advCorpSelect = this.root.querySelector('#ponto-adv-corp');
    advCorpSelect?.addEventListener('change', () => {
      this.state.pontoFilters.corporationSlug = advCorpSelect.value;
      this.renderPontoPage(this.root.querySelector('#page-content'));
    });
  ...
  defineTemplates() {
    this.templates = {
      ...
      pontoPage: (stats, pontos, filters, officers = [], roles = []) => {
        ...
        const corpBreakdown = stats.corpBreakdown || [];
        const currentPeriod = this.state.pontoPeriod || 'all';
        const currentAdvRole = this.state.pontoAdvancedRoleId || '';
        const currentCorp = this.state.pontoFilters.corporationSlug || '';

        const corpNames = { pmesp: 'PMESP', pcesp: 'PCESP', ft: 'FT', rota: 'ROTA', baep: 'BAEP', bprv: 'BPRV', cavpm: 'CAvPM' };
        const corpColors = { pmesp: '#1B52F1', pcesp: '#C62828', ft: '#2E7D32', rota: '#E65100', baep: '#4A148C', bprv: '#01579B', cavpm: '#6D4C41' };
        const corpEmojis = { pmesp: '🛡️', pcesp: '🔍', ft: '⚡', rota: '🎯', baep: '💥', bprv: '🛣️', cavpm: '🐴' };
        const periodLabels = { today: 'Hoje', week: 'Semana', month: 'Mês', all: 'Tudo', custom: 'Custom' };
        const totalCorpMs = corpBreakdown.reduce((acc, c) => acc + (c.totalMs || 0), 0) || 1;

        return `
          <!-- Period Toggle -->
          <div class="flex flex-wrap items-center gap-2 mb-6">
            <div class="flex bg-zinc-950/40 p-1 rounded-xl border border-[var(--border-subtle)] gap-0.5">
              ${['today', 'week', 'month', 'all'].map(p => `
                <button class="ponto-period-btn px-4 py-2 rounded-lg text-xs font-bold transition-all ${currentPeriod === p ? 'bg-brand-500 text-white shadow-lg' : 'text-[var(--text-muted)] hover:text-zinc-200 hover:bg-zinc-800/40'}" data-period="${p}">
                  ${periodLabels[p]}
                </button>
              `).join('')}
            </div>
            <div class="bg-[var(--card-bg)] rounded-xl border border-[var(--border-subtle)] px-3 py-1 flex items-center ml-auto">
              <i class="fas fa-shield-halved text-[var(--text-muted)] mr-2 text-xs"></i>
              <select id="ponto-adv-corp" class="bg-transparent border-0 focus:ring-0 text-xs font-semibold py-1 focus:outline-none select-clean">
                <option value="" ${currentCorp === '' ? 'selected' : ''}>Todas Corp.</option>
                <option value="pmesp" ${currentCorp === 'pmesp' ? 'selected' : ''}>🛡️ PMESP</option>
                <option value="pcesp" ${currentCorp === 'pcesp' ? 'selected' : ''}>🔍 PCESP</option>
              </select>
            </div>
            <div class="bg-[var(--card-bg)] rounded-xl border border-[var(--border-subtle)] px-3 py-1 flex items-center">
              <i class="fas fa-tags text-[var(--text-muted)] mr-2 text-xs"></i>
              <select id="ponto-adv-role" class="bg-transparent border-0 focus:ring-0 text-xs font-semibold py-1 focus:outline-none select-clean">
                <option value="" ${currentAdvRole === '' ? 'selected' : ''}>Todas Patentes</option>
                ${roles.map(r => `<option value="${r.id}" ${currentAdvRole === r.id ? 'selected' : ''}>${r.name}</option>`).join('')}
              </select>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="card-premium rounded-2xl p-5 relative overflow-hidden">
              <div class="absolute top-0 right-0 w-20 h-20 bg-brand-500/5 rounded-full -translate-y-6 translate-x-6"></div>
              <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-xl bg-brand-500/10 text-brand-400 flex items-center justify-center text-sm border border-brand-500/20"><i class="fas fa-clock"></i></div>
                <span class="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider">Horas Totais</span>
              </div>
              <p class="text-2xl font-black text-zinc-100">${ps.totalHours || 0}<span class="text-sm font-bold text-[var(--text-muted)] ml-1">h</span></p>
              <p class="text-[9px] text-[var(--text-muted)] mt-1">Período: ${periodLabels[currentPeriod]}</p>
            </div>
            <div class="card-premium rounded-2xl p-5 relative overflow-hidden">
              <div class="absolute top-0 right-0 w-20 h-20 bg-emerald-500/5 rounded-full -translate-y-6 translate-x-6"></div>
              <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-400 flex items-center justify-center text-sm border border-emerald-500/20"><i class="fas fa-users"></i></div>
                <span class="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider">Oficiais Ativos</span>
              </div>
              <p class="text-2xl font-black text-zinc-100">${ps.uniqueOfficers || 0}</p>
              <p class="text-[9px] text-emerald-500 mt-1"><i class="fas fa-circle text-[6px] mr-1 animate-pulse"></i>${ps.activeOfficers || 0} em serviço agora</p>
            </div>
            <div class="card-premium rounded-2xl p-5 relative overflow-hidden">
              <div class="absolute top-0 right-0 w-20 h-20 bg-amber-500/5 rounded-full -translate-y-6 translate-x-6"></div>
              <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-xl bg-amber-500/10 text-amber-400 flex items-center justify-center text-sm border border-amber-500/20"><i class="fas fa-chart-bar"></i></div>
                <span class="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider">Média/Oficial</span>
              </div>
              <p class="text-2xl font-black text-zinc-100">${ps.avgHoursPerOfficer || 0}<span class="text-sm font-bold text-[var(--text-muted)] ml-1">h</span></p>
              <p class="text-[9px] text-[var(--text-muted)] mt-1">Horas por oficial no período</p>
            </div>
            <div class="card-premium rounded-2xl p-5 relative overflow-hidden">
              <div class="absolute top-0 right-0 w-20 h-20 bg-violet-500/5 rounded-full -translate-y-6 translate-x-6"></div>
              <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-xl bg-violet-500/10 text-violet-400 flex items-center justify-center text-sm border border-violet-500/20"><i class="fas fa-rotate"></i></div>
                <span class="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider">Total Turnos</span>
              </div>
              <p class="text-2xl font-black text-zinc-100">${ps.totalShifts || 0}</p>
              <p class="text-[9px] text-[var(--text-muted)] mt-1">Pontos fechados no período</p>
            </div>
          </div>
          ...
          ${corpBreakdown.length > 0 ? `
            <div class="card-premium rounded-2xl p-5 mb-6">
              <h4 class="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider mb-4"><i class="fas fa-chart-pie mr-2"></i>Distribuição por Corporação</h4>
              <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-${Math.min(corpBreakdown.length, 6)} gap-3">
                ${corpBreakdown.map(c => {
                  const pct = Math.round((c.totalMs / totalCorpMs) * 100);
                  const name = corpNames[c.slug] || c.slug?.toUpperCase() || 'N/A';
                  const color = corpColors[c.slug] || '#6366f1';
                  const emoji = corpEmojis[c.slug] || '🛡️';
                  return `
                    <div class="bg-zinc-950/20 rounded-xl p-3 border border-[var(--border-subtle)]/40 hover:border-opacity-100 transition-all">
                      <div class="flex items-center justify-between mb-2">
                        <span class="text-xs font-bold text-zinc-200">${emoji} ${name}</span>
                        <span class="text-[10px] font-extrabold" style="color: ${color}">${pct}%</span>
                      </div>
                      <div class="w-full bg-zinc-900/50 h-2 rounded-full overflow-hidden mb-2">
                      ...
                      <div class="flex justify-between text-[9px] text-[var(--text-muted)]">
                        ...
                        <span>${c.officersCount} oficiais</span>
                      </div>
                    </div>
                  `;
                }).join('')}
          ...
          <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="card-premium rounded-2xl p-6 flex flex-col h-fit">
              <div class="mb-4 flex items-center justify-between border-b border-[var(--border-subtle)] pb-3">
                <div>
                  <h3 class="font-bold text-base"><i class="fas fa-trophy text-amber-500 mr-2"></i>Top 15 Patrulha</h3>
                  <p class="text-xs text-[var(--text-muted)] mt-0.5">Ranking — ${periodLabels[currentPeriod]}</p>
                </div>
              </div>
              <div class="space-y-3.5 max-h-[55vh] overflow-y-auto no-scrollbar">
                ${ranking.length
                  ? ranking.map((r, i) => {
                      ...
                      const avatarUrl = r.avatarUrl || `https://cdn.discordapp.com/embed/avatars/${parseInt(r.userId || '0') % 5}.png`;
                      return `
                        <div class="flex items-center justify-between border-b border-[var(--border-subtle)] pb-2 last:border-0 last:pb-0 ${i < 3 ? 'bg-zinc-950/20 -mx-2 px-2 py-2 rounded-xl' : ''}">
                          <div class="flex items-center min-w-0 gap-2">
                            ${trophy}
                            <img src="${avatarUrl}" class="w-7 h-7 rounded-lg border border-[var(--border-subtle)] object-cover" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'" />
                            <span class="font-semibold text-xs text-zinc-200 truncate">${r.username}</span>
                          </div>
                          <div class="text-right flex-shrink-0 ml-2">
                            <span class="text-xs font-bold text-brand-400">${r.totalHours}h</span>
                            <p class="text-[9px] text-[var(--text-muted)]">${r.shiftsCount} turnos</p>
                          </div>
                        </div>
                      `;
                    }).join('')
                  : `<p class="text-xs text-[var(--text-muted)] text-center py-6">Nenhum dado de ranking neste período.</p>`
                }
              </div>
            </div>

            <!-- Daily Activity Chart + Records -->
            <div class="xl:col-span-2 space-y-6">
              <div class="card-premium rounded-2xl p-6">
                <div class="flex items-center justify-between mb-4">
                  <div>
                    <h3 class="font-bold text-base"><i class="fas fa-chart-line text-brand-400 mr-2"></i>Atividade Diária</h3>
                    <p class="text-xs text-[var(--text-muted)] mt-0.5">Horas e turnos por dia</p>
                  </div>
                </div>
                <div class="h-56"><canvas id="ponto-daily-chart"></canvas></div>
              </div>

              <div class="card-premium rounded-2xl overflow-hidden flex flex-col">
                ...
                <div class="p-5 border-b border-[var(--border-subtle)]/70 bg-zinc-950/10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs">
                  <div class="flex flex-col gap-1.5">
                    ...
                    <div class="bg-[var(--card-bg)] rounded-xl border border-[var(--border-subtle)] px-3 py-1 flex items-center">
                      ...
                      <select id="ponto-officer" class="bg-transparent border-0 focus:ring-0 text-xs font-semibold py-1 w-full pl-0 focus:outline-none select-clean">
                        ...
                        ${officers.map(o => `<option value="${o.id}" ${filters.userId === o.id ? 'selected' : ''}>${o.displayName} (${o.username})</option>`).join('')}
                      </select>
                    </div>
                  </div>
                  <div class="flex flex-col gap-1.5">
                    ...
                    <div class="bg-[var(--card-bg)] rounded-xl border border-[var(--border-subtle)] px-3 py-1 flex items-center">
                      ...
                      <select id="ponto-role" class="bg-transparent border-0 focus:ring-0 text-xs font-semibold py-1 w-full pl-0 focus:outline-none select-clean">
                        ...
                        ${roles.map(r => `<option value="${r.id}" ${filters.roleId === r.id ? 'selected' : ''}>${r.name}</option>`).join('')}
                      </select>
                    </div>
                  </div>
                  <div class="flex flex-col gap-1.5">
                  ...
                  <div class="sm:col-span-2 md:col-span-3 flex justify-between mt-2">
                    <div class="flex gap-2">
                      <button id="export-ponto-xlsx" class="btn-soft text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:text-emerald-500"><i class="fas fa-file-excel"></i> XLSX</button>
                      <button id="export-ponto-pdf" class="btn-soft text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:text-rose-500"><i class="fas fa-file-pdf"></i> PDF</button>
                    </div>
                    <button id="ponto-apply-filter" class="bg-brand-500 hover:bg-brand-600 text-white text-xs px-6 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-md active:scale-95 transition-all"><i class="fas fa-filter"></i> Filtrar</button>
                  </div>
                </div>
                  ...
                  ? `
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto no-scrollbar">
                      ${(() => {
                        ...
                        if (filters.userId) filtered = filtered.filter(o => o.id === filters.userId);
                        if (filters.roleId) filtered = filtered.filter(o => o.roles && o.roles.includes(filters.roleId));
                        if (filters.q) { const query = filters.q.toLowerCase(); filtered = filtered.filter(o => o.displayName.toLowerCase().includes(query) || o.username.toLowerCase().includes(query) || o.id.includes(query)); }
                        if (filters.status) { const activeUserIds = new Set(pontos.filter(p => p.status === filters.status).map(p => p.userId)); filtered = filtered.filter(o => activeUserIds.has(o.id)); }
                        const statsMap = {};
                        filtered.forEach(o => { statsMap[o.id] = { durationMs: 0, shiftsCount: 0 }; });
                        pontos.forEach(p => { if (statsMap[p.userId]) { if (p.status === 'fechado' && p.durationMs) statsMap[p.userId].durationMs += p.durationMs; statsMap[p.userId].shiftsCount += 1; } });
                        if (!filtered.length) return '<div class="col-span-full text-center py-12 text-[var(--text-muted)]"><i class="fas fa-users-slash text-3xl mb-2 block opacity-30"></i>Nenhum oficial encontrado.</div>';
                        return filtered.map(o => {
                          ...
                          return '<div class="view-ponto-officer-btn cursor-pointer card-premium rounded-xl p-5 flex flex-col justify-between border border-[var(--border-subtle)] bg-[var(--card-bg)] hover:border-brand-500/50 hover:shadow-lg transition-all duration-300 relative group animate-fade-in" data-user-id="' + o.id + '">' +
                            '<div class="absolute top-4 right-4 text-zinc-500 group-hover:text-brand-400 transition-colors"><i class="fas fa-arrow-up-right-from-square text-xs"></i></div>' +
                            '<div class="flex items-center gap-3 mb-3"><img src="' + avatarUrl + '" class="w-11 h-11 rounded-xl border border-[var(--border-subtle)] object-cover bg-zinc-900" onerror="this.src=\'https://cdn.discordapp.com/embed/avatars/0.png\'" /><div class="min-w-0"><p class="font-bold text-sm text-zinc-100 truncate">' + o.displayName + '</p><p class="text-[10px] text-[var(--text-muted)] truncate">@' + o.username + '</p></div></div>' +
                            '<div class="flex flex-wrap gap-1 mb-3">' + (officerRoles.length > 0 ? officerRoles.slice(0, 3).map(r => { const color = r.color && r.color !== '#000000' ? r.color : '#6366f1'; return '<span class="px-2 py-0.5 text-[9px] font-extrabold rounded-md border" style="color: ' + color + '; background-color: ' + color + '15; border-color: ' + color + '25">' + r.name + '</span>'; }).join('') : '<span class="px-2 py-0.5 text-[9px] font-extrabold rounded-md border border-[var(--border-subtle)] text-[var(--text-muted)]">Sem Cargo</span>') + '</div>' +
                            '<div class="grid grid-cols-2 gap-2 mt-auto pt-3 border-t border-[var(--border-subtle)]/40"><div class="bg-zinc-950/20 rounded-lg p-2 text-center"><p class="text-[8px] text-[var(--text-muted)] uppercase tracking-wider font-bold">Total Horas</p><p class="text-xs font-black text-emerald-400 mt-0.5">' + totalHours.toFixed(1) + 'h</p></div><div class="bg-zinc-950/20 rounded-lg p-2 text-center"><p class="text-[8px] text-[var(--text-muted)] uppercase tracking-wider font-bold">Turnos</p><p class="text-xs font-black text-brand-400 mt-0.5">' + oStats.shiftsCount + '</p></div></div></div>';
                        }).join('');
                      })()}
                    </div>
                  `
                      ...
                      <table class="min-w-full text-xs">
                        <thead class="bg-[var(--card-bg-soft)] border-b border-[var(--border-subtle)]">
                          <tr>
                            ...
                            <th class="px-5 py-3 text-right font-bold text-[var(--text-muted)] uppercase tracking-wider text-[9px]">Duração</th>
                            <th class="px-5 py-3 text-center font-bold text-[var(--text-muted)] uppercase tracking-wider text-[9px]">Status</th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-[var(--border-subtle)]/50">
                          ${pontos.length ? pontos.slice(0, 100).map(p => {
                            const durationHours = p.status === 'fechado' ? Math.round(p.durationMs / (1000 * 60 * 60) * 100) / 100 : 0;
                            const statusBadge = p.status === 'aberto'
                              ? '<span class="px-2 py-0.5 text-[9px] font-extrabold rounded-md bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">EM SERVIÇO</span>'
                              : '<span class="px-2 py-0.5 text-[9px] font-extrabold rounded-md bg-zinc-500/10 text-zinc-400 border border-zinc-500/20">FECHADO</span>';
                            const corpLabel = (p.corporationSlug || 'pmesp').toUpperCase();
                            return `
                              <tr class="hover:bg-[var(--card-bg-soft)]/50 transition-colors">
                                <td class="px-5 py-3 font-semibold text-zinc-200">${p.username}</td>
                                <td class="px-5 py-3"><span class="px-2 py-0.5 text-[9px] font-bold rounded-md bg-brand-500/10 text-brand-400 border border-brand-500/20">${corpLabel}</span></td>
                                <td class="px-5 py-3">${this.formatDateTime(p.entrada)}</td>
                                <td class="px-5 py-3">${p.saida ? this.formatDateTime(p.saida) : '<span class="text-emerald-500 font-semibold">Em patrulha...</span>'}</td>
                                <td class="px-5 py-3 text-right font-bold ${p.status === 'fechado' ? 'text-brand-400' : 'text-zinc-500'}">${p.status === 'fechado' ? durationHours + 'h' : '---'}</td>
                                <td class="px-5 py-3 text-center">${statusBadge}</td>
                              </tr>
                            `;
                          }).join('') : `
                            <tr>
                              <td colspan="6" class="px-5 py-12 text-center text-[var(--text-muted)]">
